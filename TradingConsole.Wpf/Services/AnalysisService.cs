...
namespace TradingConsole.Wpf.Services
{
    ...
    public class AnalysisService : INotifyPropertyChanged
    {
        ...
        private MarketRegime DetermineMarketRegime(AnalysisResult result)
        {
            bool isBullTrend = result.EmaSignal15Min == "Bullish Cross";
            bool isBearTrend = result.EmaSignal15Min == "Bearish Cross";
            bool isVolatile = result.AtrSignal5Min == "Vol Expanding" || result.AtrSignal1Min == "Vol Expanding";
            bool isQuiet = result.AtrSignal5Min == "Vol Contracting" || result.AtrSignal1Min == "Low Vol";

            if (isBullTrend)
            {
                return isVolatile ? MarketRegime.TrendingBull : MarketRegime.RangeBound;
            }
            if (isBearTrend)
            {
                return isVolatile ? MarketRegime.TrendingBear : MarketRegime.RangeBound;
            }
            if (isVolatile)
            {
        ...
        {
            var regime = DetermineMarketRegime(result);
            result.MarketRegime = regime;

            var (bullishDrivers, bearishDrivers) = regime switch
            {
                MarketRegime.TrendingBull => SynthesizeTrendingBullSignal(result),
                MarketRegime.TrendingBear => SynthesizeTrendingBearSignal(result),
                MarketRegime.RangeBound => SynthesizeRangeBoundSignal(result),
                MarketRegime.Volatile => SynthesizeVolatileSignal(result),
                _ => (new List<(string, int)>(), new List<(string, int)>())
            };

            result.BullishDrivers = bullishDrivers.Select(d => $"{d.Item1} (w: {d.Item2})").ToList();
            result.BearishDrivers = bearishDrivers.Select(d => $"{d.Item1} (w: {d.Item2})").ToList();

            int bullishScore = bullishDrivers.Sum(d => d.Item2);
            int bearishScore = bearishDrivers.Sum(d => d.Item2);
            string oldSignal = result.FinalTradeSignal;
            result.ConvictionScore = bullishScore - bearishScore;

            if (result.ConvictionScore >= 5)
            {
                result.FinalTradeSignal = result.IvRank > 70 ? "High IV - Consider Bull Put Spread" : "Strong Buy (Calls)";
            }
            else if (result.ConvictionScore <= -5)
            {
                result.FinalTradeSignal = result.IvRank > 70 ? "High IV - Consider Bear Call Spread" : "Strong Sell (Puts)";
            }
            else if (result.ConvictionScore >= 3)
            {
                result.FinalTradeSignal = "Weak Buy Signal";
            }
            else if (result.ConvictionScore <= -3)
            {
                result.FinalTradeSignal = "Weak Sell Signal";
            }
            else
            {
            ...
            if (Math.Abs(result.ConvictionScore) >= 5)
            {
        ...
        private (List<(string, int)>, List<(string, int)>) SynthesizeTrendingBullSignal(AnalysisResult r)
        {
            var bullish = new List<(string, int)>();
            var bearish = new List<(string, int)>();

            if (r.InstitutionalIntent.Contains("Bullish")) bullish.Add(("Institutional Intent is Bullish", 3));
            if (r.PriceVsVwapSignal == "Above VWAP") bullish.Add(("Price above VWAP", 2));
            if (r.VwapEmaSignal5Min == "Bullish Cross") bullish.Add(("5m VWAP EMA confirms trend", 1));
            if (r.OiSignal == "Long Buildup") bullish.Add(("OI confirms new longs", 2));
            if (r.InitialBalanceSignal == "IB Extension Up") bullish.Add(("IB breakout is extending", 2));
            if (r.IvTrendSignal == "IV Contraction") bullish.Add(("IV contracting supports calm uptrend", 1));

            bool atSupport = r.MarketProfileSignal.Contains("dPOC") || r.MarketProfileSignal.Contains("dVAL");
            if (atSupport && r.ObvDivergenceSignal1Min == "Bullish Divergence")
            {
                bullish.Add(("Bullish OBV Div at Profile Support", 3));
            }
            if (atSupport && r.RsiSignal5Min == "Bullish Divergence")
            {
                bullish.Add(("Bullish RSI Div at Profile Support", 2));
            }

            if (r.CandleSignal5Min.Contains("Bearish")) bearish.Add(("Minor 5m pullback candle", 1));

            return (bullish, bearish);
        }

        private (List<(string, int)>, List<(string, int)>) SynthesizeTrendingBearSignal(AnalysisResult r)
        {
            var bullish = new List<(string, int)>();
            var bearish = new List<(string, int)>();

            if (r.InstitutionalIntent.Contains("Bearish")) bearish.Add(("Institutional Intent is Bearish", 3));
            if (r.PriceVsVwapSignal == "Below VWAP") bearish.Add(("Price below VWAP", 2));
            if (r.VwapEmaSignal5Min == "Bearish Cross") bearish.Add(("5m VWAP EMA confirms trend", 1));
            if (r.OiSignal == "Short Buildup") bearish.Add(("OI confirms new shorts", 2));
            if (r.InitialBalanceSignal == "IB Extension Down") bearish.Add(("IB breakdown is extending", 2));
            if (r.IvTrendSignal == "IV Spike Up") bearish.Add(("IV spiking confirms fear", 2));

            bool atResistance = r.MarketProfileSignal.Contains("dPOC") || r.MarketProfileSignal.Contains("dVAH");
            if (atResistance && r.ObvDivergenceSignal1Min == "Bearish Divergence")
            {
                bearish.Add(("Bearish OBV Div at Profile Resistance", 3));
            }
            if (atResistance && r.RsiSignal5Min == "Bearish Divergence")
            {
                bearish.Add(("Bearish RSI Div at Profile Resistance", 2));
            }

            if (r.CandleSignal5Min.Contains("Bullish")) bullish.Add(("Minor 5m rally candle", 1));

            return (bullish, bearish);
        }

        private (List<(string, int)>, List<(string, int)>) SynthesizeRangeBoundSignal(AnalysisResult r)
        {
            var bullish = new List<(string, int)>();
            var bearish = new List<(string, int)>();

            bool atUpperExtreme = r.MarketProfileSignal.Contains("dVAH") || r.DayRangeSignal == "Near High";
            bool atLowerExtreme = r.MarketProfileSignal.Contains("dVAL") || r.DayRangeSignal == "Near Low";

            if (atUpperExtreme)
            {
                if (r.ObvDivergenceSignal5Min == "Bearish Divergence") bearish.Add(("Bearish OBV Div at range high", 3));
                if (r.RsiSignal5Min == "Bearish Divergence") bearish.Add(("Bearish RSI Div at range high", 2));
                if (r.VolumeSignal != "Volume Burst") bearish.Add(("Low volume suggests exhaustion", 1));
            }

            if (atLowerExtreme)
            {
                if (r.ObvDivergenceSignal5Min == "Bullish Divergence") bullish.Add(("Bullish OBV Div at range low", 3));
                if (r.RsiSignal5Min == "Bullish Divergence") bullish.Add(("Bullish RSI Div at range low", 2));
                if (r.VolumeSignal != "Volume Burst") bullish.Add(("Low volume suggests exhaustion", 1));
            }

            if (r.CustomLevelSignal.Contains("Breakout") && r.VolumeSignal == "Volume Burst")
            {
                bullish.Add(("Possible range breakout with volume", 2));
            }
            if (r.CustomLevelSignal.Contains("Breakdown") && r.VolumeSignal == "Volume Burst")
            {
                bearish.Add(("Possible range breakdown with volume", 2));
            }

            return (bullish, bearish);
        }

        private (List<(string, int)>, List<(string, int)>) SynthesizeVolatileSignal(AnalysisResult r)
        {
            var bullish = new List<(string, int)>();
            var bearish = new List<(string, int)>();

            bool strongBullishConfluence = r.EmaSignal5Min == "Bullish Cross" && r.PriceVsVwapSignal == "Above VWAP" && r.VolumeSignal == "Volume Burst";
            bool strongBearishConfluence = r.EmaSignal5Min == "Bearish Cross" && r.PriceVsVwapSignal == "Below VWAP" && r.VolumeSignal == "Volume Burst";

            if (strongBullishConfluence && r.InstitutionalIntent.Contains("Bullish"))
            {
                bullish.Add(("Strong bullish confluence with Inst. backing", 4));
            }
            if (strongBearishConfluence && r.InstitutionalIntent.Contains("Bearish"))
            {
                bearish.Add(("Strong bearish confluence with Inst. backing", 4));
            }

            if (r.AtrSignal1Min == "Vol Expanding")
            {
                bullish.Add(("Caution: High Volatility", -1));
                bearish.Add(("Caution: High Volatility", -1));
            }
